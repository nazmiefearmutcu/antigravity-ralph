---
name: ralphloop
description: Start the NEVER-STOPPING Ralph engine on THIS repo. Use when the user types /ralphloop or says "keep improving until I stop" / "never stop". Launches the headless daemon (ralph start --daemon) that improves the repo FOREVER and auto-recovers from any agy stall/timeout; it does NOT stop until the user runs `ralph stop`.
---
